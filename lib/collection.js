...
const {insertBestBlock,insertAvgBlockTime} =require("../postgresDB/utils/stats");
...
Collection.prototype.update = function(id, stats, callback)
{
	...
	else
	{
		...
		else
		{
			...
			console.log("Stats in collection update>>>>>>>>>>>>>>>>>>>>>>>");
			console.log(stats);
...
{
	bestBlockNum= Math.max(this._blockchain.bestBlockNumber(), _.result(_.max(this._items, function(item) {
		// return ( !item.trusted ? 0 : item.stats.block.number );
		return ( item.stats.block.number );
	}), 'stats.block.number', 0));
	console.log("BEST BLOCK NUM>>>>>>>>>>>>>>>>>>>>>>")
	console.log(bestBlockNum)
	insertBestBlock(bestBlockNum);
	return bestBlockNum;
}
